...
config.skipFilesFromBundle.dist.push("assets/js/demo.js")
...
function package() {
  return gulp
    ...
    .pipe(replace(/\/\/ ONLY DEV(.|\n)*?\/\/ END ONLY DEV\s+/g, ''))
    .pipe(gulp.dest('./front-dashboard'))
}
...
function clean() {
  return del("./front-dashboard", {force: true});
}
...
function fileInclude() {
  return gulp
    ...
    .pipe(replace(new RegExp('(\\.+\\/)+' + "node_modules" + '\/.*\\.*', 'g'), function (match, p1) {
      ...
      return match.replace('node_modules', "./front-dashboard/dist/assets/vendor".replace("./front-dashboard/dist" + '/', ''))
    }))
    .pipe(deleteLines({
      'filters': [
    ...
    .pipe(gulp.dest("./front-dashboard/dist"))
};
...
function minCSS() {
  return gulp
    ...
    .pipe(gulp.dest("./front-dashboard/dist/assets/css"))
}
...
function minJS() {
  ...
  if (bundle.length) {
    return gulp
      ...
      .pipe(uglify())
      .pipe(gulp.dest("./front-dashboard/dist/assets/js"));
  } else {
...
function copy() {
  return gulp
    ...
    .pipe(gulp.dest("./front-dashboard/dist/assets"))
}
...
function copyNode() {
  ...
  if ([...node].length) {
    return gulp
      .src([...node])
      .pipe(gulp.dest("./front-dashboard/dist/assets/vendor"));
  } else {
    return new Promise(function (resolve, reject) {
      resolve();
    });
...
function copyDependencies() {
  for (var k in config.copyDependencies.dist) {
    ...
    gulp
      .src(path)
      .pipe(gulp.dest("./front-dashboard/dist" + '/' + config.copyDependencies.dist[k]))
  }
